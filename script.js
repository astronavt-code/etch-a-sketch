...
Create a webpage with a 16x16 grid of square divs.
    ...
    Create the divs using JavaScript. Don’t try making them by hand with copy and pasting in your HTML
     file!
    It’s best to put your grid squares inside another “container” div (which can go directly 
        in your HTML).
    ...
    “OMG, why isn’t my grid being created???”
        ...
        Go willy-nilly and add console.log statements in your JavaScript to see if it’s actually being 
        loaded.
...
function createGrid(number) {
  let numberOfDivs = number;
  // Need to clear grid if there is already divs
  clearGrid();

  for (let i = 0; i < number; i++) {
    const divCol = document.createElement("div");
    divCol.classList.add("grid-col");
    document.querySelector(".container").appendChild(divCol);
...
button.addEventListener("click", (event) => {
  ...
  //  changes color on created divs
  allGridDivs = document.querySelectorAll(".grid-div");
  allGridDivs.forEach((onediv) => {
    onediv.addEventListener("mouseover", () => {
      onediv.classList.add("change-bg");
